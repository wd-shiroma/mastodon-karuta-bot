...
var start_streaming = async function() {
    ...
    var posted = await bot.post('statuses', { status: status });
    since_id = posted.id;
    if (!interval_id) interval_id = setInterval(connect_stream, 30000);

    timeout_id = setTimeout(timeover, config.responsive);
};

var connect_stream = async function() {
    var deck = karuta.deck();
    var payload;
    statuses = await bot.get('timelines/public', {local: true, since_id: since_id});

    for(i = (statuses.length - 1); i >= 0; i--) {
        payload = statuses[i];
        since_id = payload.id;
        if (!interval_id) break;

        if (payload.account.acct === verify_credentials.acct) continue;
